...
resource "oci_core_subnet" "infisicalSubnet" {
  vcn_id              = oci_core_vcn.internal.id
  display_name        = "InternalSubnet"
  cidr_block          = "172.16.1.0/24"
  compartment_id      = "${var.compartment_id}"
  security_list_ids   = [oci_core_security_list.infisicalSecurityList.id]
}

# Firewall
resource "oci_core_security_list" "infisicalSecurityList" {
  compartment_id = "${var.compartment_id}"
  display_name   = "infisicalSecurityList"
  vcn_id         = "${oci_core_vcn.internal.id}"
...
resource "oci_core_instance" "Infisical" {
  ...
  create_vnic_details {
    subnet_id        = "${oci_core_subnet.infisicalSubnet.id}"
...
output "public_ip" {
  value = oci_core_instance.Infisical.public_ip
}